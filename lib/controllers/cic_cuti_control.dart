...
import 'package:fjghrd/controllers/cic_cuti_masal_control.dart' as cuti_masal;
...
class CicCutiControl extends GetxController {
  ...
  Future<void> loadJatah() async {
    var hasil = await AFdatabase.send(url: 'cic/jatah-cuti-tahunan?tahun=${filterTahun.value}');
    if (hasil.success) {
      listJatah.clear();
  ...
  Future<void> loadJenisKhusus() async {
    var hasil = await AFdatabase.send(url: 'cic/jenis-cuti-khusus');
    if (hasil.success) {
...
void inputJatahForm(String id, {String? defaultKaryawanId, String? defaultKaryawanNama, String? defaultTahun}) {
        ...
        builder: (ctrl) {
          return Container(
            ...
            child: Column(
              ...
              children: [
                AFwidget.formHeader('Form Jatah Cuti Tahunan CIC'),
                Padding(
  ...
  Future<void> hitungSisaJatah() async {
    if (selectedKaryawanJatah != null && selectedTahunFormJatah != null && txtJatahId.text == '') {
        var hasil = await AFdatabase.send(url: 'cic/jatah-cuti-tahunan/hitung-sisa?cic_karyawan_id=${selectedKaryawanJatah!.value}&tahun=${selectedTahunFormJatah!.value}');
        if(hasil.success) {
  ...
  Future<void> simpanJatah() async {
    try {
      ...
      var body = {
        'cic_karyawan_id': selectedKaryawanJatah!.value,
      ...
      AFwidget.loading();
      var hasil = await AFdatabase.send(
        url: txtJatahId.text == '' ? 'cic/jatah-cuti-tahunan' : 'cic/jatah-cuti-tahunan/${txtJatahId.text}',
      ...
      if (hasil.success) {
        ...
        if (Get.isRegistered<cuti_masal.CicCutiMasalControl>()) {
          if (selectedKaryawanJatah != null) {
            Get.find<cuti_masal.CicCutiMasalControl>().loadSingleInfoMasal(selectedKaryawanJatah!.value);
          }
        }
  ...
  Future<void> hapusJatah(String id) async {
    AFwidget.formHapus(
      ...
      aksi: () async {
        try {
          AFwidget.loading();
          var hasil = await AFdatabase.send(url: 'cic/jatah-cuti-tahunan/$id', methodeRequest: MethodeRequest.delete);
          Get.back();
          if (hasil.success) {
            loadJatah();
            Get.back();
    ...
    if(cuti != null) {
      ...
      for(var det in cuti.details) {
          ...
          if(kat == 'TAHUNAN') {
              ...
              // Ijin form uses TAHUNAN variables under the hood in CicCutiFormView (it checks formType == 'IJIN' and displays Tahunan checklist)
              idDetailTahunan = det['id']?.toString() ?? '';
              cekTahunan = true;
              txtAkanDiambil.text = det['lama_hari']?.toString() ?? '';
              txtKetTahunan.text = det['keterangan'] ?? '';
