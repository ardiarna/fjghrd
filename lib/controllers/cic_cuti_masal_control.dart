...
class CicCutiMasalControl extends GetxController {
  ...
  void onInit() {
    ...
    // Get year from CicCutiControl if exists
    if(Get.isRegistered<CicCutiControl>()) {
      filterTahun = Opsi(value: Get.find<CicCutiControl>().filterTahun.value, label: Get.find<CicCutiControl>().filterTahun.label);
    } else {
  ...
  Future<void> loadInfoMasal() async {
    ...
    try {
      var hasil = await AFdatabase.send(
        url: 'cic/cuti/info-masal?tahun=${filterTahun.value}',
      );
      Get.back();
      if(hasil.success) {
        listKaryawan.clear();
        for(var a in hasil.daftar) {
          listKaryawan.add(CicKaryawanCutiMasal.fromMap(a));
        }
        update();
      } else {
  ...
  Future<void> loadSingleInfoMasal(String karyawanId) async {
    try {
      var hasil = await AFdatabase.send(
        url: 'cic/cuti/info-masal?tahun=${filterTahun.value}&cic_karyawan_id=$karyawanId',
      );
      if(hasil.success && hasil.daftar.isNotEmpty) {
        var updatedKaryawan = CicKaryawanCutiMasal.fromMap(hasil.daftar.first);
  ...
  Future<DateTime?> pilihTanggalKaryawan(CicKaryawanCutiMasal k) async {
    List<Opsi> available = [];
  ...
  void checkKaryawan(CicKaryawanCutiMasal k, bool? val) {
  ...
  Future<void> simpanData() async {
    ...
    for(var k in checkedList) {
      ...
      kList.add({
        'cic_karyawan_id': k.karyawanId,
        'details': details,
      });
    }
    
    body['cic_karyawans'] = kList;

    AFwidget.loading();
    try {
      var hasil = await AFdatabase.send(
        url: 'cic/cuti/submit-masal',
