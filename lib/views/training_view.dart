...
import 'package:fjghrd/views/training_form.dart';
...
class TrainingView extends StatelessWidget {
  ...
  final TrainingControl controller = Get.put(TrainingControl());

  List<PlutoRow> _buildRows(List<Training> rowData) {
    ...
    final List<PlutoColumn> columns = [
      PlutoColumn(
        ...
        renderer: (rdrCtx) {
          return IconButton(
            onPressed: () {
              showTrainingForm(rdrCtx.row.cells['id']!.value);
            },
            icon: const Icon(
              Icons.edit_square,
            ),
          ...
          child: Container(
            ...
            child: Obx(() {
                return PlutoGrid(
                  key: UniqueKey(),
                  columns: columns,
                  rows: _buildRows(controller.listTraining),
