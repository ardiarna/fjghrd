...
class CicCutiMasalView extends StatelessWidget {
  const CicCutiMasalView({super.key});

  @override
  Widget build(BuildContext context) {
    return GetBuilder<CicCutiMasalControl>(
      init: CicCutiMasalControl(),
      builder: (controller) {
        return Scaffold(
          body: Column(
            children: [
              AFwidget.pageHeader(
                title: 'FORM CUTI MASAL CIC',
                icon: Icons.groups,
                onBack: () => Get.back(),
              ),
  ...
  Widget _buildBottomAction(CicCutiMasalControl controller) {
    return Container(
      ...
      child: GetBuilder<CicCutiMasalControl>(
        builder: (_) {
